...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with three element") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val rdd = sc.parallelize(Seq(WikipediaArticle("title", "Java Jakarta, Scala is a very important lang, Scala and Scala")))
    val res = occurrencesOfLang("Scala", rdd) == 3
    assert(res, "occurrencesOfLang given (specific) RDD with one element should equal to 1")
  }
  ...
  test("'rankLangs' should work for RDD with three elements") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "Haskell")
    val rdd = sc.parallelize(List(WikipediaArticle("1", "Scala is great"),
      WikipediaArticle("2", "Java is OK, but Scala is cooler"),
      WikipediaArticle("3", "Haskell is the best thing tho, Haskell and Haskell, best Haskell")))
    val ranked = rankLangs(langs, rdd)
    val res = ranked.head._1 == "Haskell"
    assert("Haskell" == ranked.head._1)
  }
  ...
  test("'rankLangsUsingIndex' should work for a simple RDD with three elements, three langs") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val langs = List("Scala", "Java", "Haskell")
    val articles = List(
      WikipediaArticle("1","Groovy is pretty interesting, and so is Erlang and Haskell"),
      WikipediaArticle("2","Scala and Java run on the JVM, frege which is a Haskell port also runs"),
      WikipediaArticle("3","Scala is not purely functional, but Haskell is!")
    )
    val rdd = sc.parallelize(articles)
    val index = makeIndex(langs, rdd)
    val ranked = rankLangsUsingIndex(index)
